...
export function StatsIndex() {
  ...
  async function add() {
    ...
    setName(""); await qc.invalidateQueries({ queryKey: ["seasons"] }); toast.success("Kausi lisätty");
  }
  ...
  return <div className="mx-auto max-w-4xl px-4 py-8">
    ...
    {admin.isAdmin && <div className="card-dark p-3 mt-6 flex flex-wrap gap-2 items-center"><input value={name} onChange={e => setName(e.target.value)} placeholder="Kauden nimi (esim. Kausi 2027)" className="flex-1 min-w-[200px] bg-black/70 border border-primary/30 rounded p-2 text-sm" /><input type="number" value={year} onChange={e => setYear(Number(e.target.value))} className="w-24 bg-black/70 border border-primary/30 rounded p-2 text-sm" /><button onClick={add} className="bg-primary text-primary-foreground rounded px-4 py-2 text-xs font-display uppercase tracking-widest">Lisää kausi</button></div>}
  </div>;
}
