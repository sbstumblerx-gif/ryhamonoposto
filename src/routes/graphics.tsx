...
function GraphicsPage() {
  ...
  const history = useQuery({ queryKey: ["my-graphs"], queryFn: () => historyFn(), retry: false });
  const clubs = useQuery({ queryKey: ["my-clubs"], queryFn: () => clubsFn(), retry: false });
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 pb-24">
    ...
    <section className="card-dark p-5 md:p-6">
      ...
      <button disabled={creating} onClick={create} className="mt-5 w-full md:w-auto bg-primary text-primary-foreground rounded px-6 py-3 font-display uppercase tracking-widest disabled:opacity-50">{creating ? "Luo..." : "📊 Luo grafiikka"}</button>
    </section>
    ...
    {graph && <section className="card-dark p-4 md:p-6 mt-6" id="graph-export">
      ...
      <div className="mt-4 border-t border-white/10 pt-4">
        ...
        <div className="flex flex-col md:flex-row gap-2">
          ...
          <button disabled={!clubId} onClick={async () => { try { await shareFn({ data: { graph_id: graph.id, club_id: clubId } }); toast.success("Grafiikka jaettu klubin chattiin."); } catch (e: any) { toast.error(e?.message ?? "Jakaminen epäonnistui"); } }} className="bg-primary text-primary-foreground rounded px-4 py-2 text-sm inline-flex items-center justify-center gap-2 disabled:opacity-50"><Share2 size={15} /> Jaa</button>
        </div>
        {!clubs.data?.length && <p className="text-xs text-muted-foreground mt-2">Kirjaudu sisään ja liity klubin jäseneksi, jotta voit jakaa grafiikan klubichattiin.</p>}
      </div>
    </section>}

    <section className="mt-8">
      <h2 className="font-display uppercase tracking-widest text-lg mb-3">Omat grafiikat</h2>
      {history.isLoading ? <p className="text-sm text-muted-foreground">Ladataan...</p> : !history.data?.length ? <p className="text-sm text-muted-foreground">Et ole vielä tallentanut grafiikoita.</p> : <div className="grid md:grid-cols-2 gap-3">
