...
function ProfilePage() {
  ...
  async function loadTags(id: string) {
    try {
      ...
      const { data } = await (supabase as any).from("profiles").select("club_tag_club_id").eq("id", id).maybeSingle();
  ...
  async function save(next?: { avatar_url?: string | null }) {
    ...
    setBusy(true);
    try {
      const { error } = await supabase.from("profiles").upsert({ id: uid, display_name: display, avatar_url: next && "avatar_url" in next ? next.avatar_url ?? null : avatar });
  ...
  async function pickFile(file: File) {
    ...
    try {
      ...
      setAvatar(url); await save({ avatar_url: url });
    } catch (e: any) { toast.error(e?.message ?? "Kuvan lataus epäonnistui"); }
    finally { setBusy(false); }
  }

  async function chooseTag(id: string | null) {
    setBusy(true);
    try { await setTagFn({ data: { club_id: id } }); setSelectedTag(id); toast.success(id ? "Klubitunniste valittu" : "Klubitunniste poistettu"); }
    catch (e: any) { toast.error(e?.message ?? "Klubitunnisteen valinta epäonnistui"); }
    finally { setBusy(false); }
  }

  async function saveOwnedTag(c: any, enabled: boolean) {
    setBusy(true);
    try {
      await updateTagFn({ data: { club_id: c.id, tag: c.tag ?? "", emoji: c.tag_emoji ?? null, enabled } });
      toast.success(enabled ? "Klubitunniste otettu käyttöön" : "Klubitunniste poistettu käytöstä");
      if (uid) await loadTags(uid);
  ...
  if (!uid) return <div className="mx-auto max-w-md px-4 py-10 space-y-4 text-center"><h1 className="font-display uppercase tracking-widest text-2xl text-primary">Profiili</h1><div className="hairline-red" /><p className="text-sm text-muted-foreground">Kirjaudu sisään asettaaksesi käyttäjänimen ja avatarin.</p><button onClick={() => lovable.auth.signInWithOAuth("google", { redirect_uri: window.location.origin + "/profiili" })} className="rounded bg-white text-black text-sm font-display uppercase tracking-widest px-4 py-2">Kirjaudu Googlella</button></div>;
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-10 space-y-6">
      ...
      <section className="card-dark p-4 space-y-3">
        ...
        {ownedClubs.length === 0 ? <p className="text-xs text-muted-foreground">Et omista vielä klubeja.</p> : ownedClubs.map(c => <div key={c.id} className="border border-primary/20 rounded p-3 space-y-2">
          <div className="text-sm font-display uppercase tracking-widest">{c.name}</div>
          <div className="flex gap-2"><input value={c.tag ?? ""} onChange={(e) => setClubTags(prev => prev.map(x => x.id === c.id ? { ...x, tag: e.target.value.toUpperCase().slice(0,5) } : x))} placeholder="TAG" className="w-24 bg-black/70 border border-primary/30 rounded p-2 text-sm uppercase" /><input value={c.tag_emoji ?? ""} onChange={(e) => setClubTags(prev => prev.map(x => x.id === c.id ? { ...x, tag_emoji: e.target.value.slice(0,2) } : x))} placeholder="🏁" className="w-20 bg-black/70 border border-primary/30 rounded p-2 text-sm" /></div>
          <div className="flex gap-2 flex-wrap"><button disabled={busy} onClick={() => void saveOwnedTag(c, true)} className="bg-primary text-primary-foreground rounded px-3 py-2 text-[10px] uppercase tracking-widest">Tallenna ja aktivoi</button><button disabled={busy} onClick={() => void saveOwnedTag(c, false)} className="border border-primary/40 rounded px-3 py-2 text-[10px] uppercase tracking-widest">Poista käytöstä</button></div>
        </div>)}
      </section>
      <SvAccountPanel />
    </div>
