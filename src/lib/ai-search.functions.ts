...
export const aiChat = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) => z.object({ messages: z.array(ChatMessage).min(1).max(12) }).parse(d))
  .handler(async ({ data }) => {
    const key = process.env.LOVABLE_API_KEY;
    if (!key) throw new Error("Missing LOVABLE_API_KEY");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    const [drivers, teams, races, news, seasons, media] = await Promise.all([
      supabaseAdmin.from("drivers").select("slug, name, number, flag, team_slug, content, info_card, current_team_slug, current_team_since, former_teams"),
      supabaseAdmin.from("teams").select("slug, name, flag, content, info_card, current_driver_slugs, former_lineups"),
    ...
    const systemPrompt = `Olet RyhäMonoposto-sarjan keskusteleva AI-tila. Vastaa suomeksi, keskustele luontevasti sarjan tapahtumista ja käytä vain annetun sivuston dataa sekä liitettyjä kuvia. Jos et tiedä, sano se. Mainitse tarvittaessa mihin kisaan, kuljettajaan, tiimiin, uutiseen tai tilasto-osioon tieto perustuu.`;
    const messages = [
      { role: "system", content: systemPrompt },
