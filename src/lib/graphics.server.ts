...
type Race = { name: string; slug: string; flag: string; round_number: number | null; race_date: string | null; qualifying_content: string | null; race_content: string | null; driver_of_the_day_slug?: string | null; fastest_lap_driver_slug?: string | null };
...
async function loadData() {
  ...
  const [{ data: races, error: re }, { data: drivers, error: de }, { data: teams, error: te }] = await Promise.all([
    supabaseAdmin.from("races").select("name, slug, flag, round_number, race_date, qualifying_content, race_content, driver_of_the_day_slug, fastest_lap_driver_slug" as any),
...
const empty = () => ({ points: 0, wins: 0, podiums: 0, dnf: 0, dsq: 0, dns: 0, poles: 0, starts: 0, driverOfTheDay: 0, fastestLaps: 0 });
...
function sliceRange<T>(rows: T[], range: string) { if (range === "last5") return rows.slice(-5); if (range === "last10") return rows.slice(-10); if (range === "last20") return rows.slice(-20); return rows; }
function makeSegments(points: GraphPoint[]) {
  if (!points.length) return [];
  const out: { color: string; data: GraphPoint[] }[] = []; let color = points[0]?.teamColor ?? "#9ca3af"; let data: GraphPoint[] = [];
  for (const point of points) { const next = point.teamColor ?? "#9ca3af"; if (next !== color && data.length) { const previous = data[data.length - 1]!; out.push({ color, data: [...data, point] }); data = [previous, point]; color = next; } else data.push(point); }
  if (data.length) out.push({ color, data }); return out;
}
...
export async function buildGraph(config: any) {
  ...
  let selected = races.filter(r => r.round_number !== 0 && (!season || seasonYearFromName(r.name) === season)); selected = sliceRange([...selected].sort(compareRaceOrder), config.range);
  ...
  const totals = new Map<string, ReturnType<typeof empty>>(); const series = new Map<string, GraphPoint[]>(); meta.forEach((p: any) => { totals.set(p.key, empty()); series.set(p.key, []); });
  const championships = championshipCounts(races, config.target, driverByName, teamByName, teamBySlug);

  for (const race of selected) {
    const year = seasonYearFromName(race.name); const lines = parseResultLines(race.race_content); const q = parseResultLines(race.qualifying_content);
    ...
    for (const p of meta) {
      const acc = totals.get(p.key)!; let usedTeam: Team | undefined = config.target === "drivers" ? undefined : p.team;
      if (config.target === "drivers") {
        const d = driverBySlug.get(p.key); const line = byDriver.get(p.key);
        if (line?.team) usedTeam = teamByName.get(normalizeName(line.team)) ?? usedTeam; if (!usedTeam) usedTeam = historicalTeam(d, year, teamBySlug);
        if (line) add(acc, line); if (pole.has(p.key)) acc.poles++;
        if (raceAny.driver_of_the_day_slug === p.key) acc.driverOfTheDay++;
        if (raceAny.fastest_lap_driver_slug === p.key) acc.fastestLaps++;
      } else {
        ...
        const dayDriver = driverBySlug.get(raceAny.driver_of_the_day_slug ?? ""); const fastDriver = driverBySlug.get(raceAny.fastest_lap_driver_slug ?? "");
        const dayTeam = historicalTeam(dayDriver, year, teamBySlug); const fastTeam = historicalTeam(fastDriver, year, teamBySlug);
        if (dayTeam?.slug === p.key) acc.driverOfTheDay++;
        if (fastTeam?.slug === p.key) acc.fastestLaps++;
      }
  ...
  const metricNames: Record<string, string> = { points: "Pisteet", wins: "Voitot", podiums: "Podiumit", dnf: "DNF:t", dsq: "DSQ:t", dns: "DNS:t", poles: "Paalut", starts: "Startit", championships: "Maailmanmestaruudet", driverOfTheDay: "Päivän kuljettaja", fastestLaps: "Nopeimmat kierrokset" };
