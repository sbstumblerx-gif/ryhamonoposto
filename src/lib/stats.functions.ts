...
import {
  ...
  emptyStatLine, applyResult, sortStandings, compareRaceOrder, type StatLine,
} from "./stats-compute";
...
export const getStandings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<StandingsResult> => {
    ...
    const getOrCreate = (map: Map<string, StatLine>, key: string, name: string, slug: string | null, flag: string) => {
      let row = map.get(key);
      if (!row) {
        row = emptyStatLine(key, name, slug, flag);
        map.set(key, row);
      }
      return row;
    };
    ...
    const applySpecial = (map: Map<string, StatLine>, slug: string | null, field: "driverOfTheDay" | "fastestLaps") => {
      if (!slug) return;
      const d = driverBySlug.get(slug);
      if (!d) return;
      const row = getOrCreate(map, d.slug, d.name, d.slug, d.flag);
      row[field] += 1;
    };
    ...
    for (const race of orderedRaces) {
      ...
      let seasonD = seasonDriverRows.get(year);
      if (!seasonD) { seasonD = new Map(); seasonDriverRows.set(year, seasonD); }
      let seasonT = seasonTeamRows.get(year);
      if (!seasonT) { seasonT = new Map(); seasonTeamRows.set(year, seasonT); }
      ...
      for (const session of ["qualifying", "race"] as const) {
        ...
        for (const line of lines) {
          ...
          const tRow = getOrCreate(teamRows, tKey, tName, t?.slug ?? null, t?.flag ?? "");
          const stRow = getOrCreate(seasonT, tKey, tName, t?.slug ?? null, t?.flag ?? "");
      ...
      for (let i = 0; i < specialDriverSlugs.length; i++) {
        ...
        if (!slug) continue;
        applySpecial(driverRows, slug, awardFields[i]);
        applySpecial(seasonD, slug, awardFields[i]);

        const d = driverBySlug.get(slug);
        if (!d) continue;
        const matchingLine = parseResultLines(race.race_content).find(line => normalizeName(line.driver) === normalizeName(d.name));
        const t = (matchingLine?.team ? teamByName.get(normalizeName(matchingLine.team)) : undefined)
          ?? (d.current_team_slug ? teamBySlug.get(d.current_team_slug) : undefined)
          ?? (d.team_slug ? teamBySlug.get(d.team_slug) : undefined);
        if (t) {
          const tr = getOrCreate(teamRows, t.slug, t.name, t.slug, t.flag);
          const str = getOrCreate(seasonT, t.slug, t.name, t.slug, t.flag);
          tr[awardFields[i]] += 1;
          str[awardFields[i]] += 1;
        }
      }
    ...
    const toRecord = (source: Map<number, Map<string, StatLine>>) => Object.fromEntries(
      [...source.entries()].map(([year, rows]) => [String(year), sortStandings([...rows.values()])]),
    );
