...
async function admin(): Promise<Admin> { const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); return supabaseAdmin as unknown as Admin; }
function normalizeTag(tag: string) { return tag.trim().replace(/\s+/g, "").toUpperCase(); }
...
export async function listMyClubTags(userId: string) {
  ...
  const ids = (mem ?? []).map(x => x.club_id); if (!ids.length) return [];
...
async function findClubByTag(db: Admin, tag: string) {
  const value = normalizeTag(tag);
  const { data: club } = await db.from("clubs").select("id, name, description, visibility, require_approval, tag, tag_emoji, tag_enabled, owner_id").ilike("tag", value).eq("tag_enabled", true).maybeSingle();
  if (!club) throw new Error("Klubitunnistetta ei löytynyt");
  return club;
}
export async function getClubByTag(tag: string) {
  const db = await admin(); const club = await findClubByTag(db, tag);
...
export async function joinClubByTag(userId: string, tag: string) {
  const db = await admin(); const club = await findClubByTag(db, tag);
  const { data: existing } = await db.from("club_members").select("role").eq("club_id", club.id).eq("user_id", userId).maybeSingle();
  if (existing) return { status: "member" as const, club_id: club.id };
  const { count } = await db.from("club_members").select("user_id", { count: "exact", head: true }).eq("club_id", club.id);
  if ((count ?? 0) >= 50) throw new Error("Klubi on täynnä (50 jäsentä)");
  if (club.require_approval) {
    ...
    if (staff?.length) await db.from("notifications").insert(staff.map(s => ({ user_id: s.user_id, type: "join_request", title: "Uusi liittymispyyntö", body: `Klubiin ${club.name} on uusi liittymispyyntö.`, club_id: club.id })));
...
export async function profileTagMap(userIds: string[]) {
  const db = await admin(); const out = new Map<string, { clubId: string; tag: string; emoji: string | null }>();
  if (!userIds.length) return out;
  const { data: profiles } = await db.from("profiles").select("id, club_tag_club_id").in("id", userIds).not("club_tag_club_id", "is", null);
  const clubIds = (profiles ?? []).map(p => p.club_tag_club_id).filter(Boolean) as string[]; if (!clubIds.length) return out;
  const { data: clubs } = await db.from("clubs").select("id, tag, tag_emoji, tag_enabled").in("id", clubIds).eq("tag_enabled", true);
  const byId = new Map((clubs ?? []).map(c => [c.id, c]));
  for (const p of profiles ?? []) { const c = p.club_tag_club_id ? byId.get(p.club_tag_club_id) : null; if (c?.tag) out.set(p.id, { clubId: c.id, tag: c.tag, emoji: c.tag_emoji }); }
  return out;
}
