...
export const listDrivers = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [{ data, error }, colors] = await Promise.all([
    supabaseAdmin.from("drivers").select("*").order("number", { ascending: true }),
    teamColorMap(),
  ]);
...
export const listTeams = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("teams").select("id, slug, name, flag, color_key, content, hero_media_url, logo_url, info_card, current_driver_slugs, former_lineups, engine_supplier, engine_contract_start_year, engine_contract_year").order("name");
  if (error) throw error;
  return data ?? [];
});
...
export const listRaces = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("races").select("*");
  if (error) throw error;
  return data ?? [];
});
...
export const getLatestSessionResult = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("races").select("slug, name, qualifying_content, race_content, qualifying_updated_at, race_updated_at, updated_at, created_at").order("updated_at", { ascending: false }).limit(30);
  if (error) throw error;
  const sessions = (data ?? []).flatMap((race) => [
    { slug: race.slug, title: `${race.name} aika-ajot`, content: race.qualifying_content ?? "", updatedAt: race.qualifying_updated_at ?? race.updated_at ?? race.created_at },
    { slug: race.slug, title: `${race.name} kisa`, content: race.race_content ?? "", updatedAt: race.race_updated_at ?? race.updated_at ?? race.created_at },
  ]).filter(session => session.content.trim().length > 0);
  sessions.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  const latest = sessions[0];
  if (!latest) return null;
  const summary = latest.content.replace(/!\[[^\]]*\]\([^)]*\)/g, " ").replace(/\s+/g, " ").trim().slice(0, 240);
...
export const listNews = createServerFn({ method: "GET" }).handler(async () => {
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  const { data, error } = await supabaseAdmin.from("news").select("id, slug, title, excerpt, hero_media_url, published_at").order("published_at", { ascending: false });
  if (error) throw error;
  return data ?? [];
});
...
function slugify(s: string): string {
  return s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/ä/g, "a").replace(/ö/g, "o").replace(/å/g, "a").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "item";
}

const RaceInput = z.object({
  id: z.string().uuid().optional(), name: z.string().min(1).max(200), round_number: z.number().int().min(0).max(50).nullable().optional(), flag: z.string().max(20).default(""), race_date: z.string().nullable().optional(),
...
export const upsertRace = createServerFn({ method: "POST" }).inputValidator((d: unknown) => RaceInput.parse(d)).handler(async ({ data }) => {
  await assertAdmin();
  const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
  if (data.id) {
    ...
    } as any).eq("id", data.id).select().single();
    if (error) throw error;
    return row;
  }
...
export const deleteRace = createServerFn({ method: "POST" }).inputValidator((d: unknown) => z.object({ id: z.string().uuid() }).parse(d)).handler(async ({ data }) => {
  await assertAdmin(); const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); const { error } = await supabaseAdmin.from("races").delete().eq("id", data.id); if (error) throw error; return { ok: true };
});
...
export const upsertNews = createServerFn({ method: "POST" }).inputValidator((d: unknown) => NewsInput.parse(d)).handler(async ({ data }) => { await assertAdmin(); const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); if (data.id) { const { data: row, error } = await supabaseAdmin.from("news").update({ title: data.title, excerpt: data.excerpt, content: data.content, hero_media_url: data.hero_media_url ?? null }).eq("id", data.id).select().single(); if (error) throw error; return row; } const slug = `${slugify(data.title)}-${Date.now().toString(36)}`; const { data: row, error } = await supabaseAdmin.from("news").insert({ slug, title: data.title, excerpt: data.excerpt, content: data.content, hero_media_url: data.hero_media_url ?? null }).select().single(); if (error) throw error; try { const { addFanPointsForMentionedEntities } = await import("./fan-points.server"); await addFanPointsForMentionedEntities(`${data.title}\n${data.excerpt}\n${data.content}`, 4, `news-mention:${row.id}`); } catch (e) { console.error("news fan points failed", e); } try { const { notifyFollowersOfNews } = await import("./follows.server"); await notifyFollowersOfNews(data.title, `${data.excerpt}\n${data.content}`); } catch (e) { console.error("news follower notify failed", e); } return row; });
...
export const setLiveRace = createServerFn({ method: "POST" }).inputValidator((d: unknown) => z.object({ id: z.string().uuid().nullable() }).parse(d)).handler(async ({ data }) => { await assertAdmin(); const { supabaseAdmin } = await import("@/integrations/supabase/client.server"); const { error: clearError } = await supabaseAdmin.from("races").update({ is_live: false } as any).eq("is_live", true); if (clearError) throw clearError; if (data.id) { const { error } = await supabaseAdmin.from("races").update({ is_live: true } as any).eq("id", data.id); if (error) throw error; } return { ok: true }; });
