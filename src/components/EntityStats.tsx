...
export function EntityStats({ kind, slug }: { kind: "drivers" | "teams"; slug: string }) {
  const fn = useServerFn(getStandings); const q = useQuery({ queryKey: ["standings", "all"], queryFn: () => fn({ data: { year: null } }) });
  ...
  return <div><p className="text-xs text-muted-foreground mb-3 uppercase tracking-widest">Lasketaan automaattisesti kaikista sessioista ja kilpailun palkinnoista</p><div className="grid grid-cols-2 md:grid-cols-5 gap-2">{FIELDS.map(f => <div key={f.key} className="card-dark p-3 text-center"><div className="font-display text-2xl text-primary">{row[f.key] ?? "–"}</div><div className="text-[10px] uppercase tracking-widest text-muted-foreground">{f.label}</div></div>)}</div></div>;
}
