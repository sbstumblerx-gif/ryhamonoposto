...
export function EntityStats({ kind, slug }: { kind: "drivers" | "teams"; slug: string }) {
  ...
  if (q.isError) {
    return <div className="text-sm text-muted-foreground">Tilastojen lataus epäonnistui.{" "}<button onClick={() => void q.refetch()} className="text-primary underline">Yritä uudelleen</button></div>;
  }
  ...
  return (
    <div>
      <p className="text-xs text-muted-foreground mb-3 uppercase tracking-widest">Sijoitukset lasketaan sekä koko historiasta että viimeisimmästä kilpailukaudesta</p>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
        {FIELDS.map(f => {
          const historyRank = rankRows(allRows, f.key).get(row.key) ?? 1;
          const seasonRow = lastSeasonRows.find(r => r.key === row.key || r.slug === slug);
          const seasonRank = seasonRow ? (rankRows(lastSeasonRows, f.key).get(seasonRow.key) ?? 1) : null;
          return (
            <div key={f.key} className="card-dark p-3 text-center min-h-[118px] flex flex-col items-center justify-center">
              ...
              <div className="text-[10px] text-muted-foreground leading-5">
                <div>
                  <span className={`inline-flex items-center rounded px-1.5 py-0.5 ${medalClass(historyRank)}`}>Sija {historyRank}</span>
                  <span className="ml-1">kok. hist.</span>
                </div>
                {lastSeason != null && seasonRank != null && (
                  <div>
