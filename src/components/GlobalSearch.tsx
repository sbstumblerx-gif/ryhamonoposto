...
export function GlobalSearch(){
  ...
  const searchFn=useServerFn(siteSearch);
  const [q,setQ]=useState(""); const [debounced,setDebounced]=useState(""); const [open,setOpen]=useState(false); const [filtersOpen,setFiltersOpen]=useState(false);
  const [types,setTypes]=useState<SearchType[]>([...SEARCH_TYPES]);
  useEffect(()=>{const t=setTimeout(()=>setDebounced(q.trim()),250); return()=>clearTimeout(t);},[q]);
  useEffect(()=>{setOpen(false);setFiltersOpen(false);setQ("");setDebounced("");},[pathname]);
  ...
  return <div className="relative flex-1 max-w-xl mx-1 md:mx-4">
    ...
    {(open&&debounced||filtersOpen)&&<div className="absolute left-0 right-0 top-full mt-2 z-50 rounded-xl border border-primary/40 bg-black/95 backdrop-blur-xl shadow-2xl overflow-hidden" onMouseDown={e=>e.stopPropagation()}>
      ...
      {open&&debounced&&<div className="max-h-[65vh] overflow-y-auto">{query.isLoading?<div className="p-4 text-sm text-muted-foreground">Haetaan…</div>:query.isError?<div className="p-4 text-sm text-muted-foreground">Haku epäonnistui.</div>:query.data?.length?Array.from(grouped.entries()).map(([type,rows])=><div key={type}><div className="px-3 pt-3 pb-1 text-[10px] uppercase tracking-widest text-primary/80">{labels[type]}</div>{rows?.map(r=><Link key={`${r.type}-${r.href}-${r.title}`} to={r.href} onClick={()=>setOpen(false)} className="block px-3 py-2 hover:bg-primary/10 border-t border-white/5"><div className="text-sm text-white truncate">{r.title}</div>{r.subtitle&&<div className="text-[11px] text-muted-foreground truncate">{r.subtitle}</div>}</Link>)}</div>):<div className="p-4 text-sm text-muted-foreground">Ei hakutuloksia.</div>}</div>}
    </div>}
  </div>;
}
